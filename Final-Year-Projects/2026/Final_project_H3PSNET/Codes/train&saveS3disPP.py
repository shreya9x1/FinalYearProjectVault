...
class S3DISDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        pts = data[:, 0:6]
        pts_labels = data[:, -1].astype(int)

        # If labels appear 1-based, shift to 0-based. We detect typical S3DIS (1..13)
        if pts_labels.max() > 12:
            pts_labels = pts_labels - 1
        ...
        sampled = pts[choice, :]
        sampled_labels = pts_labels[choice]

        # For per-point segmentation: return full 6-channel features (XYZ+RGB) and per-point labels
        features = torch.from_numpy(sampled.T).float()  # [6, N]
        labels_tensor = torch.from_numpy(sampled_labels).long()  # [N]
...
def train_one_epoch(model, loader, optimizer, criterion, device):
    ...
    for points, labels in tqdm(loader, desc="Training"):
        ...
        optimizer.zero_grad()
        # PointNetSegmentation outputs (B, C, N) segmentation logits
...
if __name__ == "__main__":
    ...
    # Model setup
    model = PointNetSegmentation(in_channels=6, num_classes=NUM_CLASSES).to(device)
