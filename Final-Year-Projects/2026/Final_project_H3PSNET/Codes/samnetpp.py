...
class PointNetFeaturePropagation(nn.Module):
    ...
    def forward(self, xyz1, xyz2, points1, points2):
        ...
        interpolated_points = torch.sum(index_points(points2.permute(0, 2, 1), idx) * weight.view(weight.shape[0], weight.shape[1], weight.shape[2], 1), dim=2)
...
class SAMNetPP(nn.Module):
    def __init__(self, num_classes, normal_channel=False):
        ...
        # SA1: if no normals, in_channel=0. SA1 expects 3 (relative xyz). 
        # If normals, in_channel=3. SA1 expects 3+3=6.
        ...
        # FP1 Input Channels Calculation:
        # l1_points (128) + OneHot (16) + XYZ (3) + Normals (in_channel: 0 or 3)
        fp1_in_channels = 128 + 16 + 3 + in_channel
    ...
    def forward(self, xyz, cls_label):
        ...
        if self.normal_channel:
            ...
            # FIX: Set l0_points to None so SA1 only uses relative coordinates
            l0_points = None 
            l0_xyz = xyz
        ...
        # FIX: Concatenation logic for FP1
        if self.normal_channel:
            # 16 + 3 + 3 = 22 channels
            cat_vec = torch.cat([cls_label_one_hot, l0_xyz, xyz], 1) 
        else:
            # 16 + 3 = 19 channels
            cat_vec = torch.cat([cls_label_one_hot, l0_xyz], 1)
